...
public class JFrameCounter extends JFrame {
	 ...
	 * Generated UID
	 */
	private static final long serialVersionUID = -8652729993765133587L;
	...
	public JFrameCounter() {
		try {
			...
			counter.registerObserver(textframe);
			counter.registerObserver(rectframe);
			counter.registerObserver(ovalframe);
			hist = new History(counter);
	...
	private void jButtonIncrement_actionPerformed(ActionEvent e) {
		ICounterCommand incCommand = new IncrementCommand(counter);
		incCommand.execute();
		hist.addCommand(incCommand);
	}

	private void jButtondecrement_actionPerformed(ActionEvent e) {
		ICounterCommand decCommand = new DecrementCommand(counter);
		decCommand.execute();
		hist.addCommand(decCommand);
	}
